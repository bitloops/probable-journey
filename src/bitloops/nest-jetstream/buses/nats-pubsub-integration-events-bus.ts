...
import { IHandle } from '@src/bitloops/bl-boilerplate-core/application/IHandle';
  ...
  implements Infra.EventBus.IEventBus
{
  ...
  async publish(
    ...
    const integrationEvents: Infra.EventBus.IntegrationEvent<any>[] =
      Array.isArray(eventsInput) ? eventsInput : [eventsInput];
    await Promise.all(
      ...
      (async () => {
        for await (const m of sub) {
          ...
          await this.asyncLocalStorage.run(contextData, async () => {
            return this.handleReceivedIntegrationEvent(
              handler,
              integrationEvent,
              m,
            );
  ...
  private generateHeaders(
    integrationEvents: Infra.EventBus.IntegrationEvent<any>,
  ): MsgHdrs {
    const h = headers();
  ...
  static getTopicFromEventInstance(
