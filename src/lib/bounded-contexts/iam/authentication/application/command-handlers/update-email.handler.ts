...
export class UpdateEmailHandler
    implements
    Application.ICommandHandler<UpdateEmailCommand, Promise<UpdateEmailResponse>>
{
    constructor(
    ...
    async execute(command: UpdateEmailCommand): Promise<UpdateEmailResponse> {
        ...
        if (userFound === null) {
            return fail(new ApplicationErrors.UserNotFoundApplicationError(command.userId));
        }
